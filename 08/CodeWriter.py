...
class CodeWriter(object):
    ...
    def __init__(self, fpath=None):
        ...
        if os.path.isdir(fpath): 
            # if we're dealing with a program, a collection of vm files, like "C:\Users\Adele Liu\Desktop\nand2tetris\projects\08\FunctionCalls\FibonacciElement"
            ...
            self.folder_name = fpath.split("\\")[-1] # FibonacciElement
            self.fname_out = fpath + "\\" + self.folder_name + ".asm" # "C:\...\FibonacciElement\FibonacciElement.asm"
            ...
            self.folder_name = ""
            self.fname_out = fpath.replace(".vm", ".asm") #r"C:...\07\StackTest\StackTest.asm"
    ...
    def setFileName(self, fileName):
        ...
        self.crt_fname_short = self.crt_fname.split("\\")[-1].replace(".vm","")
        if self.folder_name:
    ...
    def writeLabel(self, command):
        ...
        arg1, arg2 = command.arg1, command.arg2 # both are str type # label(arg1) end(arg2)
        #arg1 == "label"
        if self.folder_name:
        ...
        if folder_file_label in self.LABELS_CREATED:
            ...
            # label = folder_file_label + str(self.LABEL_CTR) #FibonnaciElement.Main.LOOP2
            # self.LABEL_CTR += 1
    ...
    def writeGoto(self, command):
        ...
        arg1, arg2 = command.arg1, command.arg2 # both are str type # label end, if-goto end, goto cont
        #arg1 == "goto"
        if self.folder_name:
            folder_file_label = self.folder_name+"."+self.crt_fname.split("\\")[-1].replace(".vm","")+"."+arg2.upper() #FibonnaciElement.Main.LOOP
        else:
    ...
    def writeIf(self, command):
        ...
        asm = "@SP\nAM=M-1\nD=M\n@%s\nD;JNE\n" %(label)
        # print("Label:", label, "____________________________")
        self.f_handler.write(asm)
    ...
    def writeFunction(self, command):
        ...
        # function Main.fibonacci 0      # crt_fname_short = Main,           folder_file = FibonacciElement.Main, arg1 = Main.fibonacci
        # function SimpleFunction.test 2 # crt_fname_short = SimpleFunction, folder_file = SimpleFunction,        arg1 = SimpleFunction.test
        ...
        folder_file_funcname = self.folder_file + arg1 # FibonacciElement.Main.fibonacci,  SimpleFunction.test 

        # get func_label
        if folder_file_funcname in self.LABELS_CREATED:
            func_label = self.LABELS_CREATED[folder_file_funcname]
        else:
            # func_label = folder_file_funcname + str(self.LABEL_CTR) # FibonnaciElement.Main.fibonacci: FibonacciElement.Main.fibonacci2
            # self.LABEL_CTR += 1
        ...
        # we need label counters because we are making new loop/end labels and we don't know how many loops(ends) there will be
        # loop_label = self.folder_file + ".LOOP" + str(self.LABEL_CTR)
        # self.LABEL_CTR += 1
        # end_label = self.folder_file + ".END" + str(self.LABEL_CTR)
        # self.LABEL_CTR += 1

        loop_label = func_label + ".declare.loop" # FibonnaciElement.Main.fibonacci.declare_loop
        end_label = func_label + ".declare.end"   # FibonnaciElement.Main.fibonacci.declare_end

        asm_pushlocal = "@SP\nA=M\nM=0\n@SP\nM=M+1"
        asm="(%s)\n@%s\nD=A\n(%s)\n@%s\nD;JLE\n%s\nD=D-1\n@%s\n0;JMP\n(%s)\n" % (func_label, k, loop_label, end_label, asm_pushlocal, loop_label, end_label)
        print("Label:", func_label, loop_label, end_label, "____________________________")
        self.f_handler.write(asm)
    ...
    def writeReturn(self, command):
        ...
        # can have several returns in a .vm file
        frame_label = self.folder_file + ".FRAME" + str(self.LABEL_CTR) #frame_label = "FRAME" + str(self.LABEL_CTR)
        self.LABEL_CTR += 1
        ret_label = self.folder_file + ".RET" + str(self.LABEL_CTR) #ret_label = "RET" + str(self.LABEL_CTR) 
        self.LABEL_CTR += 1
        ...
        asm += "@%s\nA=M\n0;JMP\n" %(ret_label) #????????????????????????????????????? why A=M
        # print("FRAME, RET", frame_label, ret_label)
        self.f_handler.write(asm)
    ...
    def writeInit(self):
        ...
        self.LABELS_CREATED["Sys.init"] = func_label  # "Sys.init": "FibonacciElement.Sys.init"
        ret_addrs = func_label + ".RETURN_ADDRS" + str(self.LABEL_CTR) #Fibonacci.Sys.init.RETURN_ADDRS0
        self.LABEL_CTR += 1
    ...
    def writeCall(self, command):
        "callMain.fibonacci1" # this is a line in c:\...\FibonacciElement\Main.vm and it's calling Main.fibonacci, will be converted to FibonacciElement.Main.fibonacci
        ...
        if folder_file_funcname in self.LABELS_CREATED:
            ...
            # func_label = folder_file_funcname + str(self.LABEL_CTR)
            # self.LABEL_CTR += 1
            func_label = folder_file_funcname
            self.LABELS_CREATED[folder_file_funcname] = func_label    # FibonnaciElement.Main.fibonacci: FibonacciElement.Main.fibonacci
        ...
        print("function call return address:", ret_addrs)
        self.f_handler.write(asm)
...
if __name__ == "__main__":
    fpath = r"C:\Users\Adele Liu\Desktop\nand2tetris\projects\08\FunctionCalls\fibtest"
    cw = CodeWriter(fpath)
